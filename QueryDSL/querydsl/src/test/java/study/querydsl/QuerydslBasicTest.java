...
public class QuerydslBasicTest {
    ...
    public void paging2() {
        ...
        // 총 결과 수 조회를 Optional을 사용하여 수정
        Optional<Long> totalCount = Optional.ofNullable(queryFactory
                .select(member.count())
                .from(member)
                .fetchOne()); // 결과를 Optional로 감싸기

        long total = totalCount.orElse(0L); // Optional이 비어있다면 0 반환
    ...
    public void findUserDto() {
        QMember memberSub = new QMember("memberSub");
        List<UserDto> result = queryFactory.select(Projections.bean(
                        UserDto.class,
